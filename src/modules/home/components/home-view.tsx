...
return (
  <div className="max-w-[1600px] mx-auto py-8 px-0 sm:px-4 space-y-6"> {/* تم تكبير max-w ليتناسب مع 4 أعمدة */}
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {sortedRooms.map((room: Room) => {
        ...
        const displayedPlayers = isExpanded ? sortedPlayers : sortedPlayers.slice(0, 10);
                  ...
                  <>
                        ...
                        <div key={idx} className="group relative">
                          <div className="flex justify-between items-center text-sm mb-1.5 z-10 relative">
                            <div className="flex items-center gap-3">
                              <div className="w-6 flex justify-center">
                                {isFirst ? (
                                  <Image alt="top" src="/icons/crown_king.gif" width={24} height={24} />
                                ) : (
                                  <span className={cn("text-xs font-bold", rank <= 3 ? "text-slate-400" : "text-muted-foreground")}>
                                    #{rank}
                                  </span>
                                )}
                              </div>
                              <span className={cn("font-medium truncate max-w-[100px]", isFirst && "text-primary font-bold")}>
                                {player.username}
                              </span>
                            </div>
                          ...
                          <div className="h-1 w-full bg-secondary/30 rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={cn("h-full rounded-full", isFirst ? "bg-yellow-500" : "bg-primary/70")}
                            />
                          </div>
                        </div>
                      );
                    ...
                    {sortedPlayers.length > 10 && (
                      <Button
                        ...
                        onClick={() => setExpandedRooms(prev => ({ ...prev, [room.roomId]: !isExpanded }))}
                      >
                        {isExpanded ? "إظهار أقل" : `إظهار ${sortedPlayers.length - 10} لاعبين إضافيين`}
                      </Button>
                    )}
                  </>
                )}
